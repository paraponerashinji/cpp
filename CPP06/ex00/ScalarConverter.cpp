...
/*   Updated: 2026/08/14 16:00:59 by aharder          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool inRangeInt(double nb)
{
    return (nb >= -std::numeric_limits<int>::max() && nb <=  std::numeric_limits<int>::max());
};

bool inRangeDouble(double nb)
{
    return (nb >= -std::numeric_limits<double>::max() && nb <=  std::numeric_limits<double>::max()); 
};

bool inRangeFloat(double nb)
{
...
{
    if (value[value.length() - 1] == 'f' && value != "inf")
...
void    ScalarConverter::convert(std::string value)
{
    ...
    if (!IsValid(value))
        ...
        printFloat(value);
    else if (value.find(".") == std::string::npos && inRangeInt(num))
        printInt(value);
    else
        printDouble(value);
};